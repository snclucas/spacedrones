...
public class BigDecimalMath {
  ...
  */
 public static BigDecimal GAMMA = new BigDecimal("0.577215664901532860606512090082402431"
 ...
 static public BigDecimal pow(final BigDecimal x, final BigDecimal y) {
     if (x.compareTo(BigDecimal.ZERO) < 0) {
         ...
         BigDecimal ylogx = y.multiply(logx, new MathContext(14));
 ...
 static public BigDecimal sin(final BigDecimal x)
 {
     if (x.compareTo(BigDecimal.ZERO) < 0) {
         ...
         MathContext mc = new MathContext(2 + err2prec(Math.PI, errpi));
         BigDecimal p = pi(mc);
         ...
         if (res.compareTo(p) > 0) {
            ...
            //     MathContext mcTay = new MathContext(err2prec(res.doubleValue(), xUlpDbl / k));
                 for (int i = 1;; i++) {
                     ...
                     BigDecimal corr = xpowi.divide(new BigDecimal(ifac), mc);
 ...
 static public BigDecimal asin(final BigDecimal x) {
     if (x.compareTo(BigDecimal.ONE) > 0 || x.compareTo(BigDecimal.ONE.negate()) < 0) {
         ...
         MathContext mc = new MathContext(err2prec(Math.PI, errpi));
 ...
 static public BigDecimal atan(final BigDecimal x) {
     if (x.compareTo(BigDecimal.ZERO) < 0) {
          ...
          */
         MathContext mc = new MathContext(2 + err2prec(Math.PI, eps));
 ...
 static public BigDecimal toRadians(BigDecimal deg) {
	 deg = deg.setScale(Configuration.precision, Configuration.ROUNDING_MODE);
	 return deg.divide(new BigDecimal(180, mc), mc).multiply(pi(mc), mc);
 }
