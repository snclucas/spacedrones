...
public class SensorProfileTest {
	...
	public void testSensorProfile() {
		...
		SensorProfile sensorProfile = new SensorProfile(sensorType, SensorThreshold.asW(signalThreshod), signalGain);
		assertEquals("Sensor type incorrect", SensorType.OPTICAL, sensorProfile.getSensorType());
		assertEquals("Sensor threshold incorrect", signalThreshod, sensorProfile.getSignalThreshold().getThresholdInWatts(), 0.0001);
		...
		sensorProfile = new SensorProfile(SensorType.GRAVIMETRIC, SensorThreshold.asW(newSignalThreshod), newSignalGain);
		
		assertEquals("Sensor type incorrect", SensorType.GRAVIMETRIC, sensorProfile.getSensorType());
		assertEquals("Sensor threshold incorrect", newSignalThreshod, sensorProfile.getSignalThreshold().getThresholdInWatts(), 0.0001);
