...
public class FuelSubSystemTest {
	...
	public void testFuelSubSystem() {
		...
		double volumeOfFuelTankAlone = hydrazineTank.getVolume();
		double massOfFuelTankAlone = hydrazineTank.getMass();
		...
		double massOfFuelTankPlusHydrazineFuel = hydrazineTank.getMass();
		double expectedMassOfFuelTankPlusHydrazineFuel = massOfFuelTankAlone + (hydrazineFuel.getDensity() * tankCapacity);

		double volumeOfFuelTankPlusHydrazineFuel = hydrazineTank.getVolume();
		...
		assertEquals("Expected total mass of fuel subsystem not correct", expectedTotalMassOfFuelSubsystem, fuelDeliverySystem.getMass(), 0.001);

		assertEquals("Expected total volume of fuel subsystem not correct", expectedTotalVolumeOfFuelSubsystem, fuelDeliverySystem.getVolume(), 0.001);
		...
		double expectedVolumeOfFuelSubSystemWithHydrazineAndXenonTank = expectedTotalVolumeOfFuelSubsystem + xenonTank.getVolume();
		

		assertEquals("Fuel subsystem mass (Hydrazine + Xenon) incorrect.", expectedMassOfFuelSubSystemWithHydrazineAndXenonTank, fuelDeliverySystem.getMass(), 0.001);
		assertEquals("Fuel subsystem volume (Hydrazine + Xenon) incorrect.", expectedVolumeOfFuelSubSystemWithHydrazineAndXenonTank, fuelDeliverySystem.getVolume(), 0.001);
